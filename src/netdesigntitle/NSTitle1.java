...
public class NSTitle1  extends JPanel {
	...
	private Color textColor = new Color(190,213,250);
	private Font textFontABC = new Font("Berlin Sans FB Demi", Font.PLAIN | Font.BOLD, 14);
	private Font textFontHanzi = new Font("宋体", Font.PLAIN | Font.BOLD, 14);
...
//	private Icon closeIcon = new ImageIcon(this.getClass().getResource("close.png"));
	...
	public NSTitle1(Window window) {
		this(window, null);
	}

	public NSTitle1(final Window window, String title) {
		this.window = window;
		this.setTitle(title);
		installListener();
		...
		this.addMouseListener(new MouseAdapter() {
			...
			public void mouseReleased(MouseEvent e) {
				...
				if (SwingUtilities.isRightMouseButton(e)) {
					...
					menu.show(NSTitle1.this, e.getX(), e.getY());
				}
			}
	...
	protected void paintComponent(Graphics g) {
		...
		Color a = new Color(42,104,148);
		Color b = new Color(113,153,188);
		g2d.setColor(a);
		Rectangle bounds = this.getBounds();
        
		g2d.fill(new Rectangle2D.Double(bounds.getX(), bounds.getY() + bounds.getHeight() *1/ 3, bounds.getWidth(), bounds.getHeight() *2/ 3));
		g2d.setColor(b);
		g2d.fill(new Rectangle2D.Double(bounds.getX(), bounds.getY(), bounds.getWidth(), bounds.getHeight() *1/ 3));
		int xOffset = xGap;
		if (this.icon != null) {
