...
public class Dlg_SRLGSet extends JDialog { // 代码比较乱，有时间再做修改
	...
	// public Dlg_SRLGSet(Frame
	// owner){MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM
	// super(owner,"共享风险链路组",true);MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM
	public Dlg_SRLGSet(Frame owner) {
		...
		for (int n = 0; n < LinkRGroup.SRLGroupList.size(); n++) {
			...
			case Fiber:
				...
				break;
			case WDM:
				// System.out.println(" LinkRGroup.s_lRGroup.get(n).getM_lWDMLinkList().size()"+
				// LinkRGroup.s_lRGroup.get(n).getM_lWDMLinkList().size());
			...
			// LinkRGroup.s_lRGroup.get(n).getM_lWDMLinkList().size());
			// for(int i = 0; i <
			// LinkRGroup.s_lRGroupList.get(n).getM_lOTNLinkList().size();i++){
			...
			//
			// for(int i = 0; i <
			// LinkRGroup.s_lRGroupList.get(n).getM_lSDHLinkList().size();i++){
			...
			// case ASON:
			// //System.out.println("
			// LinkRGroup.s_lRGroup.get(n).getM_lSDHLinkList().size()"+
			// LinkRGroup.s_lRGroup.get(n).getM_lWDMLinkList().size());
			// for(int i = 0; i <
			// LinkRGroup.s_lRGroupList.get(n).getM_lASONLinkList().size();i++){
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (LinkRGroup.getLinkRGroup(editId).getBelongLayer() == Layer.Fiber) {
					...
					for (int i = 0; i < LinkRGroup.getLinkRGroup(editId).getSRLGFiberLinkList().size(); i++) {
						...
						data[2] = LinkRGroup.getLinkRGroup(editId).getSRLGFiberLinkList().get(i).getFromNode()
								.getM_sName();
						data[3] = LinkRGroup.getLinkRGroup(editId).getSRLGFiberLinkList().get(i).getToNode()
								.getM_sName();
						if (teamModel.getValueAt(29, 0) == null)
							teamModel.removeRow(29);
					...
					for (int i = 0; i < FiberLink.fiberLinkList.size(); i++) {
						...
						for (j = 0; j < LinkRGroup.getLinkRGroup(editId).getSRLGFiberLinkList().size(); j++) {
							if (FiberLink.fiberLinkList.get(i).getId() == LinkRGroup.getLinkRGroup(editId)
									.getSRLGFiberLinkList().get(j).getId()) {
					...
					removeTable(linkModel);
					for (int i = 0; i < LinkRGroup.getLinkRGroup(editId).getSRLGWDMLinkList().size(); i++) {
						data[0] = LinkRGroup.getLinkRGroup(editId).getSRLGWDMLinkList().get(i).getId();
						data[1] = LinkRGroup.getLinkRGroup(editId).getSRLGWDMLinkList().get(i).getName();
						data[2] = LinkRGroup.getLinkRGroup(editId).getSRLGWDMLinkList().get(i).getFromNode()
								.getM_sName();
						data[3] = LinkRGroup.getLinkRGroup(editId).getSRLGWDMLinkList().get(i).getToNode()
								.getM_sName();
						if (teamModel.getValueAt(29, 0) == null)
							teamModel.removeRow(29);
						teamModel.insertRow(0, data);
					...
					for (int i = 0; i < WDMLink.WDMLinkList.size(); i++) {
						int j;
						for (j = 0; j < LinkRGroup.getLinkRGroup(editId).getSRLGWDMLinkList().size(); j++) {
							if (WDMLink.WDMLinkList.get(i).getId() == LinkRGroup.getLinkRGroup(editId)
									.getSRLGWDMLinkList().get(j).getId()) {

								break;
							}
						}
						if (j == LinkRGroup.getLinkRGroup(editId).getSRLGWDMLinkList().size()) {
				...
				// removeTable(linkModel);
				// for(int i = 0;i <
				// LinkRGroup.getLinkRGroup(editId).getM_lOTNLinkList().size();i++){
				// data[0] =
				// LinkRGroup.getLinkRGroup(editId).getM_lOTNLinkList().get(i).getM_nID();
				// data[1] =
				// LinkRGroup.getLinkRGroup(editId).getM_lOTNLinkList().get(i).getM_sName();
				// data[2] =
				// LinkRGroup.getLinkRGroup(editId).getM_lOTNLinkList().get(i).getM_cFromNode().getM_sName();
				// data[3] =
				// LinkRGroup.getLinkRGroup(editId).getM_lOTNLinkList().get(i).getM_cToNode().getM_sName();
				// if(teamModel.getValueAt(29,0)==null)
				// teamModel.removeRow(29);
				// teamModel.insertRow(0, data);
				//
				...
				// if(WDMLink.s_lOTNLinkList.get(i).getM_nID() ==
				// LinkRGroup.getLinkRGroup(editId).getM_lOTNLinkList().get(j).getM_nID()){
				//
				// break;
				// }
				// }
				...
				// removeTable(linkModel);
				// for(int i = 0;i <
				// LinkRGroup.getLinkRGroup(editId).getM_lASONLinkList().size();i++){
				// data[0] =
				// LinkRGroup.getLinkRGroup(editId).getM_lASONLinkList().get(i).getM_nID();
				// data[1] =
				// LinkRGroup.getLinkRGroup(editId).getM_lASONLinkList().get(i).getM_sName();
				// data[2] =
				// LinkRGroup.getLinkRGroup(editId).getM_lASONLinkList().get(i).getM_cFromNode().getM_sName();
				// data[3] =
				// LinkRGroup.getLinkRGroup(editId).getM_lASONLinkList().get(i).getM_cToNode().getM_sName();
				// if(teamModel.getValueAt(29,0)==null)
				// teamModel.removeRow(29);
				// teamModel.insertRow(0, data);
				//
				...
				// int j;
				// for(j =
				// 0;j<LinkRGroup.getLinkRGroup(editId).getM_lASONLinkList().size();j++){
				// if(SDHLink.s_lASONLinkList.get(i).getM_nID() ==
				// LinkRGroup.getLinkRGroup(editId).getM_lASONLinkList().get(j).getM_nID()){
				//
				// break;
				// }
				// }
				...
				// removeTable(linkModel);
				// for(int i = 0;i <
				// LinkRGroup.getLinkRGroup(editId).getM_lSDHLinkList().size();i++){
				// data[0] =
				// LinkRGroup.getLinkRGroup(editId).getM_lSDHLinkList().get(i).getM_nID();
				// data[1] =
				// LinkRGroup.getLinkRGroup(editId).getM_lSDHLinkList().get(i).getM_sName();
				// data[2] =
				// LinkRGroup.getLinkRGroup(editId).getM_lSDHLinkList().get(i).getM_cFromNode().getM_sName();
				// data[3] =
				// LinkRGroup.getLinkRGroup(editId).getM_lSDHLinkList().get(i).getM_cToNode().getM_sName();
				// if(teamModel.getValueAt(29,0)==null)
				// teamModel.removeRow(29);
				// teamModel.insertRow(0, data);
				//
				...
				// if(SDHLink.s_lSDHLinkList.get(i).getM_nID() ==
				// LinkRGroup.getLinkRGroup(editId).getM_lSDHLinkList().get(j).getM_nID()){
				//
				// break;
				// }
				// }
					...
					if (status == 1) {// 新建
						if (linkLayer.getSelectedIndex() == 0) {
							...
							for (int i = 0; i < getRow(teamModel); i++) {
								srg.getTemplrg().addlFiberLinkList(FiberLink
										.getFiberLink((Integer.parseInt(String.valueOf(teamModel.getValueAt(i, 0))))));
							}
							...
							for (int i = 0; i < getRow(teamModel); i++) {
								srg.getTemplrg().addlWDMLinkList(WDMLink
										.getWDMLink((Integer.parseInt(String.valueOf(teamModel.getValueAt(i, 0))))));
							}
							// JOptionPane.showMessageDialog(null,"如有需要请手动添加相应SDH层SRLG组！");
						}
						...
						// for(int i = 0;i < getRow(teamModel);i++ ){
						// srg.getTemplrg().addlWDMLinkList(WDMLink.getOTNLink((Integer.parseInt(String.valueOf(teamModel.getValueAt(i,
						// 0))))));
						// }
						//// JOptionPane.showMessageDialog(null,"如有需要请手动添加相应SDH层SRLG组！");
						// }
						...
						// for(int i = 0;i < getRow(teamModel);i++ ){
						// srg.getTemplrg().addlSDHLinkList(SDHLink.getASONLink((Integer.parseInt(String.valueOf(teamModel.getValueAt(i,
						// 0))))));
						// }
						//// JOptionPane.showMessageDialog(null,"如有需要请手动添加相应SDH层SRLG组！");
						// }
						...
						// for(int i = 0;i < getRow(teamModel);i++ ){
						// srg.getTemplrg().addlSDHLinkList(SDHLink.getSDHLink((Integer.parseInt(String.valueOf(teamModel.getValueAt(i,
						// 0))))));
						//
						// }
						//// JOptionPane.showMessageDialog(null,"如有需要请手动添加相应SDH层SRLG组！");
						...
						if (LinkRGroup.getLinkRGroup(editId).getBelongLayer() == Layer.Fiber) {
							...
							for (int m = 0; m < LinkRGroup.getLinkRGroup(editId).getSRLGFiberLinkList().size();) {
								LinkRGroup.getLinkRGroup(editId).dellFiberLinkList(
										LinkRGroup.getLinkRGroup(editId).getSRLGFiberLinkList().get(m));
								if (LinkRGroup.getLinkRGroup(editId).getSRLGFiberLinkList().size() == 0)
									break;
							}
							// 删除不了最后一个???????????新建不认层,在新建情况下，上下不对应怎么刷新
							// JOptionPane.showMessageDialog(null,
							// LinkRGroup.getLinkRGroup(editId).getM_lFiberLinkList().size());
							for (int i = 0; i < getRow(teamModel); i++) {
								LinkRGroup.getLinkRGroup(editId).addlFiberLinkList(FiberLink
										.getFiberLink((Integer.parseInt(String.valueOf(teamModel.getValueAt(i, 0))))));
							}
						} else if (LinkRGroup.getLinkRGroup(editId).getBelongLayer() == Layer.WDM) {
							srlgLayer.setSelectedIndex(2);
							for (int m = 0; m < LinkRGroup.getLinkRGroup(editId).getSRLGWDMLinkList().size();) {
								LinkRGroup.getLinkRGroup(editId)
										.delWDMLinkList(LinkRGroup.getLinkRGroup(editId).getSRLGWDMLinkList().get(m));
								if (LinkRGroup.getLinkRGroup(editId).getSRLGWDMLinkList().size() == 0)
									break;
							}
							for (int i = 0; i < getRow(teamModel); i++) {
								LinkRGroup.getLinkRGroup(editId).addlWDMLinkList(WDMLink
										.getWDMLink((Integer.parseInt(String.valueOf(teamModel.getValueAt(i, 0))))));
							}
						}
						// else if(LinkRGroup.getLinkRGroup(editId).getM_eLayer()==Layer.OTN){
						// srlgLayer.setSelectedIndex(3);
						// for(int m = 0;m <
						// LinkRGroup.getLinkRGroup(editId).getM_lOTNLinkList().size();){
						...
						// }
						// for(int i = 0;i < getRow(teamModel);i++ ){
						// LinkRGroup.getLinkRGroup(editId).addlWDMLinkList(WDMLink.getOTNLink((Integer.parseInt(String.valueOf(teamModel.getValueAt(i,
						// 0))))));
						// }
						// }
						// else if(LinkRGroup.getLinkRGroup(editId).getM_eLayer()==Layer.ASON){
						// srlgLayer.setSelectedIndex(4);
						// for(int m = 0;m <
						// LinkRGroup.getLinkRGroup(editId).getM_lASONLinkList().size();){
						...
						// }
						// for(int i = 0;i < getRow(teamModel);i++ ){
						// LinkRGroup.getLinkRGroup(editId).addlSDHLinkList(SDHLink.getASONLink((Integer.parseInt(String.valueOf(teamModel.getValueAt(i,
						// 0))))));
						// }
						// }
						// else if(LinkRGroup.getLinkRGroup(editId).getM_eLayer()==Layer.SDH){
						// srlgLayer.setSelectedIndex(5);
						// for(int m = 0;m <
						// LinkRGroup.getLinkRGroup(editId).getM_lSDHLinkList().size();){
						...
						// }
						// for(int i = 0;i < getRow(teamModel);i++ ){
						// LinkRGroup.getLinkRGroup(editId).addlSDHLinkList(SDHLink.getSDHLink((Integer.parseInt(String.valueOf(teamModel.getValueAt(i,
						// 0))))));
						// }
						// }
					}
		...
		delete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
		//		srg.setEnabled(true);//2017.11.24  wb
				editor.setEnabled(false);
				delete.setEnabled(false);
				Vector data = new Vector();
				...
				for (int i = 0; i < getId(srlgModel, srlgTable.getSelectedRows()).length; i++) {
					...
					/*
					 * else if(linkLayer.getSelectedIndex()==2){ for(int j = 0; j <
					 * it.getM_lOTNLinkList().size();){ WDMLink a = it.getM_lOTNLinkList().get(j);
					 * it.delWDMLinkList(a); Object[] objectdata =new Object[4];
					 * objectdata[0]=a.getId(); objectdata[1]=a.getName();
					 * objectdata[2]=a.getFromNode().getM_sName();
					 * objectdata[3]=a.getToNode().getM_sName(); if(linkModel.getValueAt(29,
					 * 0)==null) linkModel.removeRow(29); linkModel.insertRow(getRow(linkModel),
					 * objectdata); } }
					 */
					// else if(linkLayer.getSelectedIndex()==3){
					...
					// }
					// }
					LinkRGroup.SRLGroupList
							.remove(LinkRGroup.getLinkRGroup(getId(srlgModel, srlgTable.getSelectedRows())[i]));
				}
		...
		srlgTable.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (teamTable.getValueAt(0, 0) == null && srlgTable.getValueAt(srlgTable.getSelectedRow(), 0) != null
						&& !LinkRGroup
								.getLinkRGroup(Integer
										.parseInt(String.valueOf(srlgModel.getValueAt(srlgTable.getSelectedRow(), 0))))
								.isNatrue()
						&& !in.isEnabled()) {// 用ID获得SRG判断getNature为false才可
...
//				if (srg.isEnabled()) {
//					LinkRGroup.automation();// 测试用
//					for(LinkRGroup lg:LinkRGroup.SRLGroupList) {
//						if(lg.getBelongLayer().equals(Layer.WDM)) {
//							System.out.println(lg.getSRLGWDMLinkList());
//						}
//					}
//					refreshsrlgTable();
//					//srg.setEnabled(false);
		...
		for (int n = 0; n < LinkRGroup.SRLGroupList.size(); n++) {
			...
			for (int i = 0; i < count - 1; i++) {
				for (int j = i + 1; j < count; j++) {
					if (String.valueOf(srlgModel.getValueAt(i, 0)) != null
							&& String.valueOf(srlgModel.getValueAt(j, 0)) != null) {
						if (String.valueOf(srlgModel.getValueAt(i, 0))
								.equals((String.valueOf(srlgModel.getValueAt(j, 0))))) {
	...
	public void refreshsrlgTable() {
		...
		if (srlgLayer.getSelectedIndex() == 0) {
			for (int n = 0; n < LinkRGroup.SRLGroupList.size(); n++) {
				...
				// case OTN:
				// //System.out.println("
				// LinkRGroup.s_lRGroupList.get(n).getM_lWDMLinkList().size()"+
				// LinkRGroup.s_lRGroupList.get(n).getM_lWDMLinkList().size());
				// for(int i = 0; i <
				// LinkRGroup.SRLGroupList.get(n).getM_lOTNLinkList().size();i++){
				...
				// if(String.valueOf(srlgModel.getValueAt(i,
				// 0))!=null&&String.valueOf(srlgModel.getValueAt(j, 0))!=null){
				// if(String.valueOf(srlgModel.getValueAt(i,
				// 0)).equals((String.valueOf(srlgModel.getValueAt(j, 0))))){
				// Vector vector=new Vector();
				// srlgModel.removeRow(--index);
				// if(srlgModel.getValueAt(98, 0)== null){
				// srlgModel.insertRow(99, vector);
				...
				// case ASON:
				// //System.out.println("
				// LinkRGroup.s_lRGroupList.get(n).getM_lSDHLinkList().size()"+
				// LinkRGroup.s_lRGroupList.get(n).getM_lWDMLinkList().size());
				// for(int i = 0; i <
				// LinkRGroup.s_lRGroupList.get(n).getM_lASONLinkList().size();i++){
				// buffer.append(LinkRGroup.s_lRGroupList.get(n).getM_lASONLinkList().get(i).getM_sName());
...
//				}
		...
		// for(int
		// j=0;j<LinkRGroup.s_lRGroupList.get(i).getM_lOTNLinkList().size();j++){
		...
		// for(int
		// j=0;j<LinkRGroup.s_lRGroupList.get(i).getM_lASONLinkList().size();j++){
		...
		// for(int
		// j=0;j<LinkRGroup.s_lRGroupList.get(i).getM_lSDHLinkList().size();j++){
