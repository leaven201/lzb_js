...
//import dataControl.Exclear;
import dateBase.ExpansionDataBase;
import enums.Layer;
...
//    public static TrafficGroup trafficgroup; // 规划业务组
	...
	while (it.hasNext()) {
		...
		if (trf.getProtectLevel().equals(TrafficLevel.PERMANENT11)
		    ...
		    Route routeP = RWAMain(trf.getFromNode(), trf.getToNode(), trf.getM_eLayer(), Hop_Lenght,returnList);
    ...
    /*
     * 路由算法总入口，所有调用算法都从这里开始 输入：from,to,计算所在层layer,算法类型type，算路结果保存在returnList;
     * =1跳数 =2 长度 =3 bsp =4 综合最小 返回：void
     */
    ...
    /*
     * 计算次优路： 
     * 策略：当前路由计算失败，且不为空，则必为资源分配失败，那么只需要将资源分配失败的链路删除后再进行一次重路由，成功则继续分配资源
     * 失败则返回
     */
