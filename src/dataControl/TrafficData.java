...
//	if (Traffic.trafficList != null && Traffic.trafficList.size() != 0)
	...
	/*public static void addTraffic(int rankId, CommonNode fromNode, CommonNode toNode, TrafficRate rate, int traNum,
			TrafficLevel protectLevel, boolean isElectricalCrossConnection, boolean isShare, int idOfTrafficGroup,
			TrafficGroupType TraGroType) {
		...
		Traffic t = new Traffic(id, rankId, fromNode, toNode, rate, traNum, protectLevel, isElectricalCrossConnection,
				isShare, idOfTrafficGroup, TraGroType, MustPassNode, MustAvoidNode, MustPassLink, MustAvoidLink,
				isHaveTraffic, existRoute, existWaveLength);
